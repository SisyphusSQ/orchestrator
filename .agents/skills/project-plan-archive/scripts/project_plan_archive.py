...
def parse_candidate(repo: Path, path: Path) -> Candidate | MalformedCandidate | None:
    ...
    if path.is_symlink():
        return MalformedCandidate(path.as_posix(), relative_path, path.name, "symlink_plan")
    try:
    ...
    current_issue = extract_issue(text, "issue_id")
    selected_issue = current_issue or execution_issue or master_issue
    selected_issue_source = None
    if current_issue:
        selected_issue_source = "issue_id"
...
def git_tracked_files(repo: Path) -> list[Path]:
    try:
        result = subprocess.run(
            ["git", "-C", repo.as_posix(), "ls-files", "-z"],
            check=True,
            capture_output=True,
            text=True,
        )
    except (FileNotFoundError, subprocess.CalledProcessError) as exc:
        raise SystemExit("cannot read tracked references; archive requires a Git repository") from exc
...
def text_scan_files(repo: Path) -> list[Path]:
    ...
    for path in git_tracked_files(repo):
        if path.is_file() and not path.is_symlink() and path.resolve().is_relative_to(repo) and is_text_file(path):
            seen.add(path)
            paths.append(path)

    for pattern in [".agents/runs/**/*.md", ".agents/state/**/*.md"]:
        for path in sorted(repo.glob(pattern)):
            if path.is_file() and not path.is_symlink() and path.resolve().is_relative_to(repo) and path not in seen and is_text_file(path):
                seen.add(path)
                paths.append(path)
...
def apply_rewrites(previews: list[RewritePreview], moved: dict[Path, Path]) -> None:
    for preview in previews:
        write_text(moved.get(preview.path, preview.path), preview.updated_text)


def archive_repo(
    repo: str | Path, done_issues: list[str], *, write: bool,
    done_plans: list[str] | None = None,
) -> dict[str, Any]:
    repo_path = ensure_repo(repo)
    for path in (repo_path / ".agents", repo_path / PLANS_ROOT):
        if path.is_symlink():
            raise SystemExit("plan directory must not be a symlink")
    inspection = inspect_repo(repo_path)
    done = {issue_key(issue) for issue in done_issues}
    done.discard(None)

    eligible: list[Candidate] = []
    skipped_missing_issue: list[Candidate] = []
    ...
    explicit_plans = set(done_plans or [])
    unknown = explicit_plans - candidates_by_path.keys()
    if unknown:
        raise SystemExit("--done-plan must name a current date plan: " + ", ".join(sorted(unknown)))

    for candidate in candidates_by_path.values():
        for parent in (candidate.target_path, *candidate.target_path.parents):
            if parent.is_symlink():
                raise SystemExit(f"archive target must not be a symlink: {parent}")
            if parent == repo_path:
                break
        ...
        if selected_key is None:
            if candidate.relative_path in explicit_plans:
                eligible.append(candidate)
            else:
                skipped_missing_issue.append(candidate)
    ...
    if write:
        ...
        apply_rewrites(rewrites, {item.path: item.target_path for item in movable})
    ...
    return {
        ...
        "skipped_missing_issue": [candidate.as_dict() for candidate in skipped_missing_issue],
...
def build_parser() -> argparse.ArgumentParser:
    ...
    archive_parser.add_argument("--done-plan", action="append", default=[],
                                help="explicitly completed issue-less plan, relative to repo")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "archive":
        emit(archive_repo(args.repo, args.done_issue, write=args.write,
                          done_plans=args.done_plan), args.json)
        return 0
