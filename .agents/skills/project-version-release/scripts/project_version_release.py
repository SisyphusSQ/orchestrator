...
PLAIN_VERSION_FILES = {"VERSION", "version.txt"}
...
def read_text(path: Path) -> str:
    if path.is_symlink():
        raise SystemExit(f"refusing symlink file: {path}")
...
def normalize_changed(paths: list[str]) -> list[str]:
    return [p.strip().removeprefix("./") for p in paths if p.strip()]
...
def classify(paths: list[str], issue: str | None = None, release_intent: bool = False) -> dict[str, Any]:
    ...
    if any(Path(p).name in PLAIN_VERSION_FILES for p in changed):
        classification = "version-bump"
        reasons.append("plain version file changed; confirm the version value in its diff")

    review_files = [p for p in changed if Path(p).name in VERSION_FILE_HINTS
                    and Path(p).name not in PLAIN_VERSION_FILES]
    if review_files:
        reasons.append("manifest changed; inspect its version field before deciding release impact")
    ...
    return {
        ...
        "version_review_files": review_files,
        "reasons": reasons,
    }
...
def version_bump(repo: Path, version: str, files: list[str], write: bool) -> dict[str, Any]:
    validate_semver(version)
    if not files:
        raise SystemExit("provide at least one --file (VERSION or version.txt)")
    selected: list[Path] = []
    for rel in files:
        path = repo / rel
        if Path(rel).is_absolute() or ".." in Path(rel).parts or path.name not in PLAIN_VERSION_FILES:
            raise SystemExit("version-bump only supports repo-relative VERSION or version.txt; use native tools for manifests")
        if not path.resolve().is_relative_to(repo) or any(p.is_symlink() for p in (path, *path.parents)):
            raise SystemExit(f"version file must not escape the repository or use symlinks: {rel}")
        if not path.is_file():
            raise SystemExit(f"missing version file: {rel}")
        validate_semver(read_text(path).strip())
