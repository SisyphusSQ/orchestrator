...
- optimization
  - Migrate the Go CLI to Cobra v1.10.2, with native subcommands, grouped help and shell completion under `cmd/orchestrator`. Preserve `-c`, `cli`, old command aliases and registered single-hyphen long flags; reject mixed command selectors and unexpected arguments. Help, version and completion now skip runtime initialization, and help is written to stdout. See `docs/executing-via-command-line.md` for compatibility details.
