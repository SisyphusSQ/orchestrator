...
// defaultDiscoveryQueue is a channel of deduplicated instanceKey-s
// that were requested for discovery.  It can be continuously updated
// as discovery process progresses.
var defaultDiscoveryQueue *Queue
var deadInstancesDiscoveryQueue *Queue
var discoveryRunning atomic.Bool
var snapshotDiscoveryKeys chan instmodel.InstanceKey
...
func init() {
	snapshotDiscoveryKeys = make(chan instmodel.InstanceKey, 10)
...
func handleDiscoveryRequests() {
			...
			for {
				...
				if !IsLeaderOrActive() {
					log.Debugf("Node apparently demoted. Skipping discovery of %+v. "+
						"Remaining queue size: %+v", instanceKey, defaultDiscoveryQueue.QueueLen())
					defaultDiscoveryQueue.Release(instanceKey)
					continue
				}
	...
	if config.Config.Topology.Discovery.DeadMaxConcurrency > 0 {
		...
		deadInstancesDiscoveryQueue = defaultDiscoveryQueue
	}
}
...
func DiscoverInstance(instanceKey instmodel.InstanceKey) {
	if instinventory.InstanceIsForgotten(&instanceKey) {
	...
	_, _ = instresolve.ResolveInstanceKey(&instanceKey)
	...
	instance, found, err := instinventory.ReadInstanceContext(ctx, &instanceKey)
	...
	for _, replicaKey := range append(instance.ReplicationGroupMembers.GetInstanceKeys(), instance.Replicas.GetInstanceKeys()...) {
		...
		dead, recheck := instdiscovery.DeadInstancesFilter.InstanceRecheckNeeded(&replicaKey)
		if dead {
			if recheck {
			...
			defaultDiscoveryQueue.Push(replicaKey)
		}
	}
	// Investigate master:
	if instance.MasterKey.IsValid() {
		if !instmodel.FiltersMatchInstanceKey(&instance.MasterKey, config.Config.Topology.Discovery.IgnoreMasterHostnames) {
			...
			if dead {
				...
				defaultDiscoveryQueue.Push(instance.MasterKey)
			}
		}
	}
}

// QueueSnapshotKey schedules a legacy snapshot key for discovery after restore.
func QueueSnapshotKey(instanceKey instmodel.InstanceKey) {
	snapshotDiscoveryKeys <- instanceKey
}
...
func onHealthTick() {
	...
	instanceKeys, err := instinventory.ReadOutdatedInstanceKeys()
	if err != nil {
		log.Errore(err)
	}
	...
	if !wasAlreadyElected {
		...
		go instmaintenance.ExpireMaintenance()
	}
	...
	if len(instanceKeys) > 0 {
		for _, instanceKey := range instanceKeys {
			if instanceKey.IsValid() {
				dead, recheck := instdiscovery.DeadInstancesFilter.InstanceRecheckNeeded(&instanceKey)
				if dead {
					if recheck {
					...
					defaultDiscoveryQueue.Push(instanceKey)
				}
			}
		}
	}
...
func InjectPseudoGTIDOnWriters() error {
	...
	for i := range rand.Perm(len(instances)) {
		...
		go func() {
			if injected, _ := instreplication.CheckAndInjectPseudoGTIDOnWriter(instance); injected {
...
func SubmitMastersToKvStores(clusterName string, force bool) (kvPairs []*kv.KVPair, submittedCount int, err error) {
	kvPairs, err = instinventory.GetMastersKVPairs(clusterName)
...
func ContinuousDiscovery(ctx context.Context) error {
	...
	instresolve.LoadHostnameResolveCache()
	...
	for {
		...
		case <-instancePollTicker.C:
			go func() {
				...
				// as instance poll
				if IsLeaderOrActive() {
					go instcluster.UpdateClusterAliases()
					go instdowntime.ExpireDowntime()
		...
		case <-caretakingTicker.C:
			...
			go func() {
				if IsLeaderOrActive() {
					go instinventory.RecordInstanceCoordinatesHistory()
					go instinventory.ReviewUnseenInstances()
					go instinventory.InjectUnseenMasters(func(key *instmodel.InstanceKey) bool {
						_, skipped, _ := instdiscovery.ReadTopologyInstanceBufferable(key, false, nil)
						return skipped
					})

					go instinventory.ForgetLongUnseenInstances()
					go instcluster.ForgetLongUnseenClusterAliases()
					go instinventory.ForgetUnseenInstancesDifferentlyResolved()
					go instresolve.ForgetExpiredHostnameResolves()
					go instresolve.DeleteInvalidHostnameResolves()
					go instinventory.ResolveUnknownMasterHostnameResolves()
					go instmaintenance.ExpireMaintenance()
					go instcandidate.ExpireCandidateInstances()
					go instresolve.ExpireHostnameUnresolve()
					go instcluster.ExpireClusterDomainName()
					go instaudit.ExpireAudit()
					go instequivalence.ExpireMasterPositionEquivalence()
					go instpool.ExpirePoolInstances()
					go instresolve.FlushNontrivialResolveCacheToDatabase()
					go instinventory.ExpireInjectedPseudoGTID()
					go instinventory.ExpireStaleInstanceBinlogCoordinates()
					go process.ExpireNodesHistory()
					go process.ExpireAccessTokens()
					go process.ExpireAvailableNodes()
					go recovery.ExpireFailureDetectionHistory()
					go recovery.ExpireTopologyRecoveryHistory()
					go recovery.ExpireTopologyRecoveryStepsHistory()
					...
					go instresolve.LoadHostnameResolveCache()
				}
			}()
		...
		case <-snapshotTopologiesTick:
			go func() {
				if IsLeaderOrActive() {
					go instinventory.SnapshotTopologies()
				}
			}()
		}
	}
...
func discoverSeededAgents() {
	for seededAgent := range agent.SeededAgents {
		instanceKey := &instmodel.InstanceKey{Hostname: seededAgent.Hostname, Port: int(seededAgent.MySQLPort)}
		go instdiscovery.ReadTopologyInstance(instanceKey)
	}
}
