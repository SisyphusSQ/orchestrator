...
func TestQueueCountsEachKeyOnce(t *testing.T) {
	q := &Queue{queue: make(chan instmodel.InstanceKey, 4), queuedKeys: map[instmodel.InstanceKey]time.Time{}, consumedKeys: map[instmodel.InstanceKey]time.Time{}}
	key := instmodel.InstanceKey{Hostname: "fixture", Port: 3306}
	q.Push(key)
	q.Push(key)
...
func TestMetricsScrapeDoesNotWaitForQueueLock(t *testing.T) {
	...
	q.Push(instmodel.InstanceKey{Hostname: "fixture", Port: 3306})
	q.Lock()
	defer q.Unlock()
