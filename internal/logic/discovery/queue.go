...
// Package discovery owns the discovery queue and its continuous runtime.
package discovery
...
type Queue struct {
	...
	queue        chan instmodel.InstanceKey
	queuedKeys   map[instmodel.InstanceKey]time.Time
	consumedKeys map[instmodel.InstanceKey]time.Time
...
func (q *Queue) Push(key instmodel.InstanceKey) {
	q.Lock()
	defer q.Unlock()
...
func (q *Queue) Consume() instmodel.InstanceKey {
	q.Lock()
	queue := q.queue
	q.Unlock()
...
func (q *Queue) Release(key instmodel.InstanceKey) {
	q.Lock()
	defer q.Unlock()
