...
func NewTopologyRecovery(replicationAnalysis analysis.ReplicationAnalysis) *TopologyRecovery {
	...
	topologyRecovery.PolicyRevision, topologyRecovery.HookAssignmentRevision, _ = recoverypolicy.Revisions(context.Background(), replicationAnalysis.ClusterDetails.ClusterName)
...
func executeHookPhaseContext(ctx context.Context, phase, description string, topologyRecovery *TopologyRecovery) error {
	clusterName := ""
	if topologyRecovery != nil {
		clusterName = topologyRecovery.AnalysisEntry.ClusterDetails.ClusterName
	}
	hooks, _, err := recoverypolicy.EffectiveHooks(ctx, clusterName)
	if err != nil {
		return fmt.Errorf("resolve %s hooks: %w", description, err)
	}
...
func recoverDeadMasterInBinlogServerTopology(topologyRecovery *TopologyRecovery) (promotedReplica *instmodel.Instance, err error) {
	failedMasterKey := &topologyRecovery.AnalysisEntry.AnalyzedInstanceKey

	var promotedBinlogServer *instmodel.Instance

	_, promotedBinlogServer, err = instregroup.RegroupReplicasBinlogServers(failedMasterKey, true)
	if err != nil {
	...
	promotedReplica, err = instregroup.GetCandidateReplicaOfBinlogServerTopology(&promotedBinlogServer.Key)
	if err != nil {
		return promotedReplica, log.Errore(err)
	}
	...
	promotedReplica, err = instreplication.StartReplicationUntilMasterCoordinates(&promotedReplica.Key, &promotedBinlogServer.ExecBinlogCoordinates)
	if err != nil {
		return promotedReplica, log.Errore(err)
	}
	promotedReplica, err = instreplication.StopReplication(&promotedReplica.Key)
	...
	promotedBinlogServer, err = instreplication.SkipToNextBinaryLog(&promotedBinlogServer.Key)
	if err != nil {
		return promotedReplica, log.Errore(err)
	}
	promotedBinlogServer, err = instrelocation.Repoint(&promotedBinlogServer.Key, &promotedReplica.Key, modeldomain.GTIDHintDeny)
	if err != nil {
		return nil, log.Errore(err)
	}
		...
		for i, binlogServerReplica := range binlogServerReplicas {
			...
			postponedFunction := func() error {
				...
				if binlogServerReplica.ExecBinlogCoordinates.SmallerThan(&promotedBinlogServer.ExecBinlogCoordinates) {
					binlogServerReplica, err = instreplication.StartReplicationUntilMasterCoordinates(&binlogServerReplica.Key, &promotedBinlogServer.ExecBinlogCoordinates)
					if err != nil {
						return err
					}
				}
				_, err = instrelocation.Repoint(&binlogServerReplica.Key, &promotedReplica.Key, modeldomain.GTIDHintDeny)
				return err
			}
...
func recoverDeadMasterContext(ctx context.Context, topologyRecovery *TopologyRecovery, candidateInstanceKey *instmodel.InstanceKey, skipProcesses bool) (recoveryAttempted bool, promotedReplica *instmodel.Instance, lostReplicas []*instmodel.Instance, err error) {
	...
	case MasterRecoveryGTID:
		{
			...
			lostReplicas, _, cannotReplicateReplicas, promotedReplica, err = instregroup.RegroupReplicasGTID(failedInstanceKey, true, false, nil, &topologyRecovery.PostponedFunctionsContainer, promotedReplicaIsIdeal)
		}
	case MasterRecoveryPseudoGTID:
		{
			...
			lostReplicas, _, _, cannotReplicateReplicas, promotedReplica, err = instregroup.RegroupReplicasPseudoGTIDIncludingSubReplicasOfBinlogServers(failedInstanceKey, true, nil, &topologyRecovery.PostponedFunctionsContainer, promotedReplicaIsIdeal)
		}
	...
	if promotedReplica != nil && len(lostReplicas) > 0 && recoverypolicy.Current(analysisEntry.ClusterDetails.ClusterName).DetachLostReplicasAfterMasterFailover {
		postponedFunction := func() error {
			...
			for _, replica := range lostReplicas {
				instrelocation.DetachReplicaMasterHost(&replica.Key)
			}
			return nil
		}
...
func MasterFailoverGeographicConstraintSatisfied(analysisEntry *analysis.ReplicationAnalysis, suggestedInstance *instmodel.Instance) (satisfied bool, dissatisfiedReason string) {
	policy := recoverypolicy.Current(analysisEntry.ClusterDetails.ClusterName)
...
func SuggestReplacementForPromotedReplica(topologyRecovery *TopologyRecovery, deadInstanceKey *instmodel.InstanceKey, promotedReplica *instmodel.Instance, candidateInstanceKey *instmodel.InstanceKey) (replacement *instmodel.Instance, actionRequired bool, err error) {
...
func replacePromotedReplicaWithCandidate(topologyRecovery *TopologyRecovery, deadInstanceKey *instmodel.InstanceKey, promotedReplica *instmodel.Instance, candidateInstanceKey *instmodel.InstanceKey) (*instmodel.Instance, error) {
	...
	if candidateInstance.MasterKey.Equals(&promotedReplica.Key) {
		...
		candidateInstance, err = instrelocation.TakeMaster(&candidateInstance.Key, topologyRecovery.Type == CoMasterRecovery)
		if err != nil {
			return promotedReplica, log.Errore(err)
		}
		...
		relocateReplicasFunc := func() error {
			...
			relocatedReplicas, _, _, err := instrelocation.RelocateReplicas(&promotedReplica.Key, &candidateInstance.Key, "")
...
func checkAndRecoverDeadMaster(analysisEntry analysis.ReplicationAnalysis, candidateInstanceKey *instmodel.InstanceKey, forceInstanceRecovery bool, skipProcesses bool) (recoveryAttempted bool, topologyRecovery *TopologyRecovery, err error) {
	policy := recoverypolicy.Current(analysisEntry.ClusterDetails.ClusterName)
	...
	overrideMasterPromotion := func() (*instmodel.Instance, error) {
		...
		if policy.SQLThreadPromotionPolicy == "wait" && !promotedReplica.SQLThreadUpToDate() {
			...
			if _, err := instreplication.WaitForSQLThreadUpToDate(&promotedReplica.Key, 0, 0); err != nil {
	...
	if promotedReplica != nil {
		...
		if policy.ApplyMySQLPromotionAfterMasterFailover || analysisEntry.CommandHint == analysis.GracefulMasterTakeoverCommandHint {
			...
			{
				_, err := instrelocation.ResetReplicationOperation(&promotedReplica.Key)
				if err != nil {
		...
		if policy.MasterFailoverDetachReplicaMasterHost {
			postponedFunction := func() error {
				...
				instrelocation.DetachReplicaMasterHost(&promotedReplica.Key)
				return nil
			}
...
func isValidAsCandidateSiblingOfIntermediateMaster(intermediateMasterInstance *instmodel.Instance, sibling *instmodel.Instance) bool {
	...
	if instregroup.IsBannedFromBeingCandidateReplica(sibling) {
		return false
	}
...
func isGenerallyValidAsWouldBeMaster(replica *instmodel.Instance, requireLogReplicationUpdates bool) bool {
	...
	if instregroup.IsBannedFromBeingCandidateReplica(replica) {
		return false
	}
...
func canTakeOverPromotedServerAsMaster(wantToTakeOver *instmodel.Instance, toBeTakenOver *instmodel.Instance) bool {
...
func GetCandidateSiblingOfIntermediateMaster(topologyRecovery *TopologyRecovery, intermediateMasterInstance *instmodel.Instance) (*instmodel.Instance, error) {
...
func RecoverDeadIntermediateMaster(topologyRecovery *TopologyRecovery, skipProcesses bool) (successorInstance *instmodel.Instance, err error) {
...
func RecoverDeadIntermediateMasterContext(ctx context.Context, topologyRecovery *TopologyRecovery, skipProcesses bool) (successorInstance *instmodel.Instance, err error) {
	...
	relocateReplicasToCandidateSibling := func() {
		...
		relocatedReplicas, candidateSibling, errs, err := instrelocation.RelocateReplicas(failedInstanceKey, &candidateSiblingOfIntermediateMaster.Key, "")
	...
	if !recoveryResolved {
		...
		lostReplicas, _, _, _, regroupPromotedReplica, regroupError := instregroup.RegroupReplicas(failedInstanceKey, true, nil, nil)
		if regroupError != nil {
	...
	if !recoveryResolved {
		...
		relocatedReplicas, masterInstance, errs, err := instrelocation.RelocateReplicas(failedInstanceKey, &analysisEntry.AnalyzedInstanceMasterKey, "")
		topologyRecovery.AddError(err)
		topologyRecovery.AddErrors(errs)
...
func RecoverDeadReplicationGroupMemberWithReplicas(topologyRecovery *TopologyRecovery, skipProcesses bool) (successorInstance *instmodel.Instance, err error) {
	...
	if !skipProcesses {
		if err := executeHookPhaseContext(context.Background(), "pre_failover", "PreFailoverProcesses", topologyRecovery); err != nil {
			return nil, topologyRecovery.AddError(err)
		}
	}
	...
	relocatedReplicas, successorInstance, errs, err := instrelocation.RelocateReplicas(failedGroupMemberInstanceKey, candidateGroupMemberInstanceKey, "")
	topologyRecovery.AddErrors(errs)
...
// Returns true when action was taken.
func checkAndRecoverDeadIntermediateMaster(analysisEntry analysis.ReplicationAnalysis, candidateInstanceKey *instmodel.InstanceKey, forceInstanceRecovery bool, skipProcesses bool) (bool, *TopologyRecovery, error) {
...
func RecoverDeadCoMaster(topologyRecovery *TopologyRecovery, skipProcesses bool) (promotedReplica *instmodel.Instance, lostReplicas []*instmodel.Instance, err error) {
...
func RecoverDeadCoMasterContext(ctx context.Context, topologyRecovery *TopologyRecovery, skipProcesses bool) (promotedReplica *instmodel.Instance, lostReplicas []*instmodel.Instance, err error) {
	...
	case MasterRecoveryGTID:
		{
			lostReplicas, _, cannotReplicateReplicas, promotedReplica, err = instregroup.RegroupReplicasGTID(failedInstanceKey, true, false, nil, &topologyRecovery.PostponedFunctionsContainer, nil)
		}
	case MasterRecoveryPseudoGTID:
		{
			lostReplicas, _, _, cannotReplicateReplicas, promotedReplica, err = instregroup.RegroupReplicasPseudoGTIDIncludingSubReplicasOfBinlogServers(failedInstanceKey, true, nil, &topologyRecovery.PostponedFunctionsContainer, nil)
		}
	}
	topologyRecovery.AddError(err)
	lostReplicas = append(lostReplicas, cannotReplicateReplicas...)
	...
	if promotedReplica != nil && !promotedReplica.Key.Equals(otherCoMasterKey) {
		_, err = instrelocation.DetachReplicaMasterHost(&promotedReplica.Key)
...
// Returns true when action was taken.
func checkAndRecoverDeadCoMaster(analysisEntry analysis.ReplicationAnalysis, candidateInstanceKey *instmodel.InstanceKey, forceInstanceRecovery bool, skipProcesses bool) (bool, *TopologyRecovery, error) {
	policy := recoverypolicy.Current(analysisEntry.ClusterDetails.ClusterName)
	failedInstanceKey := &analysisEntry.AnalyzedInstanceKey
	if !(forceInstanceRecovery || analysisEntry.ClusterDetails.HasAutomatedMasterRecovery) {
		return false, nil, nil
...
func checkAndRecoverNonWriteableMaster(analysisEntry analysis.ReplicationAnalysis, candidateInstanceKey *instmodel.InstanceKey, forceInstanceRecovery bool, skipProcesses bool) (recoveryAttempted bool, topologyRecovery *TopologyRecovery, err error) {
	if !recoverypolicy.Current(analysisEntry.ClusterDetails.ClusterName).RecoverNonWriteableMaster {
		return false, nil, nil
	}
...
func checkAndRecoverLockedSemiSyncMaster(analysisEntry analysis.ReplicationAnalysis, candidateInstanceKey *instmodel.InstanceKey, forceInstanceRecovery bool, skipProcesses bool) (recoveryAttempted bool, topologyRecovery *TopologyRecovery, err error) {
	policy := recoverypolicy.Current(analysisEntry.ClusterDetails.ClusterName)
	topologyRecovery, err = AttemptRecoveryRegistration(&analysisEntry, true, true)
	if topologyRecovery == nil {
...
func checkAndRecoverMasterWithTooManySemiSyncReplicas(analysisEntry analysis.ReplicationAnalysis, candidateInstanceKey *instmodel.InstanceKey, forceInstanceRecovery bool, skipProcesses bool) (recoveryAttempted bool, topologyRecovery *TopologyRecovery, err error) {
	topologyRecovery, err = AttemptRecoveryRegistration(&analysisEntry, true, true)
	if topologyRecovery == nil {
...
func checkAndRecoverGenericProblem(analysisEntry analysis.ReplicationAnalysis, candidateInstanceKey *instmodel.InstanceKey, forceInstanceRecovery bool, skipProcesses bool) (bool, *TopologyRecovery, error) {
	return false, nil, nil
}
...
func checkAndRecoverDeadGroupMemberWithReplicas(analysisEntry analysis.ReplicationAnalysis, candidateInstanceKey *instmodel.InstanceKey, forceInstanceRecovery bool, skipProcesses bool) (bool, *TopologyRecovery, error) {
...
func checkAndExecuteFailureDetectionProcesses(analysisEntry analysis.ReplicationAnalysis, skipProcesses bool) (detectionRegistrationSuccess bool, processesExecutionAttempted bool, err error) {
	...
	err = executeHookPhaseContext(context.Background(), "failure_detection", "OnFailureDetectionProcesses", NewTopologyRecovery(analysisEntry))
	return true, true, err
}

func getCheckAndRecoverFunction(analysisCode analysis.AnalysisCode, analyzedInstanceKey *instmodel.InstanceKey) (
	checkAndRecoverFunction func(analysisEntry analysis.ReplicationAnalysis, candidateInstanceKey *instmodel.InstanceKey, forceInstanceRecovery bool, skipProcesses bool) (recoveryAttempted bool, topologyRecovery *TopologyRecovery, err error),
...
func executeCheckAndRecoverFunction(analysisEntry analysis.ReplicationAnalysis, candidateInstanceKey *instmodel.InstanceKey, forceInstanceRecovery bool, skipProcesses bool) (recoveryAttempted bool, topologyRecovery *TopologyRecovery, err error) {
	...
	if !skipProcesses {
		if topologyRecovery.SuccessorKey == nil {
			...
			_ = executeHookPhaseContext(context.Background(), "post_unsuccessful_failover", "PostUnsuccessfulFailoverProcesses", topologyRecovery)
		} else {
			// Execute general post failover processes
			instdowntime.EndDowntime(topologyRecovery.SuccessorKey)
			_ = executeHookPhaseContext(context.Background(), "post_failover", "PostFailoverProcesses", topologyRecovery)
		}
	}
...
func CheckAndRecover(specificInstance *instmodel.InstanceKey, candidateInstanceKey *instmodel.InstanceKey, skipProcesses bool) (recoveryAttempted bool, promotedReplicaKey *instmodel.InstanceKey, err error) {
...
func ForceExecuteRecovery(analysisEntry analysis.ReplicationAnalysis, candidateInstanceKey *instmodel.InstanceKey, skipProcesses bool) (recoveryAttempted bool, topologyRecovery *TopologyRecovery, err error) {
...
func ForceMasterTakeover(clusterName string, destination *instmodel.Instance) (topologyRecovery *TopologyRecovery, err error) {
...
func getGracefulMasterTakeoverDesignatedInstance(clusterMasterKey *instmodel.InstanceKey, designatedKey *instmodel.InstanceKey, clusterMasterDirectReplicas []*instmodel.Instance, auto bool) (designatedInstance *instmodel.Instance, err error) {
	if designatedKey == nil {
		...
		designatedInstance, _, _, _, _, err = instregroup.GetCandidateReplica(clusterMasterKey, false)
		...
		if _, err := instreplication.StartReplication(&designatedInstance.Key); err != nil {
...
func GracefulMasterTakeover(clusterName string, designatedKey *instmodel.InstanceKey, auto bool) (topologyRecovery *TopologyRecovery, promotedMasterCoordinates *instmodel.BinlogCoordinates, err error) {
	clusterMasters, err := instinventory.ReadClusterMaster(clusterName)
	if err != nil {
	...
	if instregroup.IsBannedFromBeingCandidateReplica(designatedInstance) {
	...
	if len(clusterMasterDirectReplicas) > 1 {
		...
		relocatedReplicas, _, _, err := instrelocation.RelocateReplicas(&clusterMaster.Key, &designatedInstance.Key, "")
	...
	if err := executeHookPhaseContext(context.Background(), "pre_graceful_takeover", "PreGracefulTakeoverProcesses", preGracefulTakeoverTopologyRecovery); err != nil {
	...
	if designatedInstance, _, err = instreplication.WaitForExecBinlogCoordinatesToReach(&designatedInstance.Key, demotedMasterSelfBinlogCoordinates, time.Duration(recoverypolicy.Current(clusterName).ReasonableMaintenanceReplicationLagSeconds)*time.Second); err != nil {
		return nil, nil, err
	}
	...
	if auto {
		_, startReplicationErr := instreplication.StartReplication(&clusterMaster.Key)
		if err == nil {
			err = startReplicationErr
		}
	}
	_ = executeHookPhaseContext(context.Background(), "post_graceful_takeover", "PostGracefulTakeoverProcesses", topologyRecovery)
