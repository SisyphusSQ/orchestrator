...
func TestTopologyRecoveryFromTypedRowPreservesNullableFields(t *testing.T) {
	row := modeldomain.TopologyRecoveryRecord{
		RecoveryID:            42,
		UID:                   "recovery-42",
		...
		Analysis:              string(analysis.DeadMaster),
	...
	if recovery.AnalysisEntry.AnalyzedInstanceKey.Hostname != row.Hostname || recovery.AnalysisEntry.Analysis != analysis.DeadMaster {
...
func TestFailureAndBlockedRecoveryRowsHandleMissingRelatedIDs(t *testing.T) {
	detection := failureDetectionFromRow(modeldomain.FailureDetectionRecord{
		DetectionID:       7,
		Hostname:          "failed.example",
		Port:              3306,
	...
	blocked := blockedRecoveryFromRow(modeldomain.BlockedRecoveryRecord{
		Hostname:           "failed.example",
		Port:               3306,
