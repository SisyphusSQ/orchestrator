...
func topologyRecoveryFromRow(row modeldomain.TopologyRecoveryRecord) *TopologyRecovery {
	topologyRecovery := NewTopologyRecovery(analysis.ReplicationAnalysis{})
	...
	topologyRecovery.AnalysisEntry.AnalyzedInstanceKey = instmodel.InstanceKey{Hostname: row.Hostname, Port: row.Port}
	topologyRecovery.AnalysisEntry.Analysis = analysis.AnalysisCode(row.Analysis)
...
func failureDetectionFromRow(row modeldomain.FailureDetectionRecord) *TopologyRecovery {
	...
	failureDetection.AnalysisEntry.AnalyzedInstanceKey = instmodel.InstanceKey{Hostname: row.Hostname, Port: row.Port}
	failureDetection.AnalysisEntry.Analysis = analysis.AnalysisCode(row.Analysis)
...
func blockedRecoveryFromRow(row modeldomain.BlockedRecoveryRecord) BlockedTopologyRecovery {
	return BlockedTopologyRecovery{
		FailedInstanceKey:    instmodel.InstanceKey{Hostname: row.Hostname, Port: row.Port},
		ClusterName:          row.ClusterName,
		Analysis:             analysis.AnalysisCode(row.Analysis),
...
func AttemptFailureDetectionRegistration(analysisEntry *analysis.ReplicationAnalysis) (registrationSuccessful bool, err error) {
...
func acknowledgeInstanceFailureDetection(instanceKey *instmodel.InstanceKey) error {
...
func RegisterBlockedRecoveries(analysisEntry *analysis.ReplicationAnalysis, blockingRecoveries []*TopologyRecovery) error {
...
// This also implied clearing their active period, which in turn enables further recoveries on those topologies
func AcknowledgeInstanceRecoveries(instanceKey *instmodel.InstanceKey, owner string, comment string) (countAcknowledgedEntries int64, err error) {
...
// This also implied clearing their active period, which in turn enables further recoveries on those topologies
func AcknowledgeInstanceCompletedRecoveries(instanceKey *instmodel.InstanceKey, owner string, comment string) (countAcknowledgedEntries int64, err error) {
	return acknowledgeRecoveries(owner, comment, metadata.RecoveryAcknowledgementFilter{
...
func writeResolveRecovery(topologyRecovery *TopologyRecovery) error {
	var successorKeyToWrite instmodel.InstanceKey
...
func ReadInActivePeriodSuccessorInstanceRecovery(instanceKey *instmodel.InstanceKey) ([]*TopologyRecovery, error) {
	return readRecoveries(metadata.RecoveryReadFilter{
...
func ReadRecentlyActiveInstanceRecovery(instanceKey *instmodel.InstanceKey) ([]*TopologyRecovery, error) {
	return readRecoveries(metadata.RecoveryReadFilter{
