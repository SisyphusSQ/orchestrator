...
// Column 仅描述主键迁移需要的数据库结构，不参与业务传输。
type Column struct {
	Name            string
	Kind            string
	NotNull         bool
...
func QuoteIdentifier(name string) string { return quoteMetadataIdentifier(name) }

func Columns(ctx context.Context, database *sql.DB, table string) ([]Column, error) {
	return metadataColumns(ctx, database, table)
}
...
func migrateMetadataIDs(ctx context.Context, database *sql.DB) error {
	...
	if _, err := execContext(ctx, database, `INSERT IGNORE INTO orchestrator_schema_migrations (migration_id, applied_at) VALUES (?, NOW())`, metadataschema.UpgradeMigrationPending); err != nil {
		return err
	}
	for _, table := range tables {
		columns, err := metadataColumns(ctx, database, table)
...
func migrateSQLiteTableID(ctx context.Context, database *sql.DB, table string, columns []Column) error {
	tx, err := database.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
...
func prepareMetadataIDMigration(ctx context.Context, database *sql.DB, layout Layout) error {
	if layout == LayoutLegacy {
		if err := DeployStatements(ctx, database, generateSQLBase); err != nil {
			return err
		}
		return DeployStatements(ctx, database, generateSQLPatches)
	}
 ...
 FROM orchestrator_schema_migrations`, metadataschema.PreviousCanonicalMigration, metadataschema.PreviousCanonicalMigration+"-pending", metadataschema.UpgradeMigrationPending).Scan(&completed, &pending, &upgrading); err != nil {
	...
	if completed == 0 && (pending > 0 || layout == LayoutUpgrade) {
		return DeployCanonicalStatements(ctx, database, metadataschema.StatementsV1())
	}
	return nil
}
