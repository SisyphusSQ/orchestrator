...
func TestQueryDynamicRowsPreservesColumnsNullAndContext(t *testing.T) {
	...
	seen := false
	err := QueryDynamicRowsContext(ctx, database, `select id, value from dynamic_fixture`, func(row modeldomain.DynamicRow) error {
		seen = true
	...
	cancel()
	if err := QueryDynamicRowsContext(canceled, database, `select id from dynamic_fixture`, func(modeldomain.DynamicRow) error { return nil }); !errors.Is(err, context.Canceled) {
...
func TestNamedResultDataRoundTripsNullSnapshotCells(t *testing.T) {
	...
	var decoded modeldomain.NamedResultData
