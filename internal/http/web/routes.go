...
// API serves the React control plane while preserving the existing Web URLs.
type API struct {
	URLPrefix string
	assets    fs.FS
}

// New creates the Web adapter. A nil asset filesystem uses the embedded build.
func New(urlPrefix string, assets fs.FS) *API {
	return &API{URLPrefix: urlPrefix, assets: assets}
}

// Config contains only public UI capabilities, never server credentials.
type Config struct {
...
func (web *API) AccessToken(_ transport.Params, r transport.Responder, req *http.Request, resp http.ResponseWriter, _ transport.Principal) {
	if err := authz.AuthenticateToken(req.URL.Query().Get("publicToken"), resp); err != nil {
		presenter.WriteJSON(r, http.StatusBadRequest, &contract.Response{Code: contract.ERROR, Message: err.Error()})
		return
	}
	r.Redirect(web.URLPrefix + "/")
}

func (web *API) Index(_ transport.Params, r transport.Responder) {
...
func (web *API) Bootstrap(_ transport.Params, r transport.Responder, req *http.Request, resp http.ResponseWriter, user transport.Principal) {
	resp.Header().Set("Cache-Control", "no-store")
	presenter.WriteJSON(r, http.StatusOK, Config{
...
func (web *API) Page(resp http.ResponseWriter, req *http.Request) {
	assets := web.assets
	if assets == nil {
...
func (web *API) registerWebRequest(m *transport.Router, path string, handler transport.Handler) {
...
func (web *API) RegisterRequests(m *transport.Router) {
	...
	m.Get(web.URLPrefix+"/api/web-config", httpraft.ReverseProxy, web.Bootstrap)

	web.RegisterDebug(m)
}

// RegisterDebug adds handlers for /debug/vars (expvar) and /debug/pprof (net/http/pprof) support
func (web *API) RegisterDebug(m *transport.Router) {
