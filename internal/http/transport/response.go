...
// BusinessError reports whether a JSON response represents a failed business
// operation. Application response types can implement this contract without
// coupling the transport adapter to a concrete API package.
type BusinessError interface {
	IsBusinessError() bool
}

func (response *response) JSON(status int, value any) {
	if result, ok := value.(BusinessError); ok && result.IsBusinessError() {
