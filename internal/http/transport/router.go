// Package transport adapts application HTTP handlers to the concrete router.
package transport
...
// Route describes one application-level route before transport-compatible
// trailing-slash and HEAD variants are expanded.
type Route struct {
	Method string
	Path   string
}
...
func (router *Router) registerLogicalRoute(method, path string, handlers ...Handler) {
	...
	router.logicalRoutes = append(router.logicalRoutes, Route{Method: method, Path: path})
...
// LogicalRoutes returns a snapshot of application-level route declarations.
func (router *Router) LogicalRoutes() []Route {
	return slices.Clone(router.logicalRoutes)
}

// StaticMounts returns a snapshot of the configured static route prefixes.
func (router *Router) StaticMounts() []string {
	return slices.Clone(router.staticMounts)
}

// HasRoute reports whether an exact transport route is registered. It accounts
// for the internal parameter-name normalization used to avoid Gin conflicts.
func (router *Router) HasRoute(method, path string) bool {
	enginePath, _ := normalizeEnginePath(path)
	_, found := router.registered[method+" "+enginePath]
	return found
}
