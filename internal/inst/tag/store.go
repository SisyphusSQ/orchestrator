...
func PutInstanceTag(instanceKey *instmodel.InstanceKey, tag *Tag) (err error) {
	return metadata.PutInstanceTag(context.Background(), instanceKey.Hostname, instanceKey.Port, tag.TagName, tag.TagValue)
}

func Untag(instanceKey *instmodel.InstanceKey, tag *Tag) (tagged *instmodel.InstanceKeyMap, err error) {
	if tag == nil {
	...
	tagged = instmodel.NewInstanceKeyMap()
	...
	for _, row := range rows {
		key, _ := instresolve.NewInstanceKey(row.Hostname, row.Port)
		tagged.AddKey(*key)
	}
	if err != nil {
		return tagged, log.Errore(err)
	}
	instaudit.AuditOperation("delete-instance-tag", instanceKey, tag.String())
	return tagged, nil
}

func ReadInstanceTag(instanceKey *instmodel.InstanceKey, tag *Tag) (tagExists bool, err error) {
...
func InstanceTagExists(instanceKey *instmodel.InstanceKey, tag *Tag) (tagExists bool, err error) {
	return ReadInstanceTag(instanceKey, &Tag{TagName: tag.TagName})
}

func ReadInstanceTags(instanceKey *instmodel.InstanceKey) (tags []*Tag, err error) {
	tags = []*Tag{}
...
func GetInstanceKeysByTag(tag *Tag) (tagged *instmodel.InstanceKeyMap, err error) {
	if tag == nil {
