...
func ProbeInstanceCommit(instanceKey *InstanceKey) error {
	db, err := topology.Open(instanceKey.Hostname, instanceKey.Port)
	if err != nil {
		return err
	}
...
func SetSemiSyncReplica(instanceKey *InstanceKey, enableReplica bool) (*Instance, error) {
	...
	if instance.ReplicationIOThreadRuning {
		...
		ExecuteInstanceCommand(instanceKey, instance.QSP.stop_slave_io_thread())
		if err := ExecuteInstanceCommand(instanceKey, instance.QSP.start_slave_io_thread()); err != nil {
			return instance, log.Errore(err)
		}
	}
	return ReadTopologyInstance(instanceKey)
...
func StopReplicationNicely(instanceKey *InstanceKey, timeout time.Duration) (*Instance, error) {
	...
	err = ExecuteInstanceCommand(instanceKey, instance.QSP.stop_slave())
	if err != nil {
...
func StartReplication(instanceKey *InstanceKey) (*Instance, error) {
	...
	err = ExecuteInstanceCommand(instanceKey, instance.QSP.start_slave())
	if err != nil {
		return instance, log.Errore(err)
	}
...
func StartReplicationUntilMasterCoordinates(instanceKey *InstanceKey, masterCoordinates *BinlogCoordinates) (*Instance, error) {
	...
	err = ExecuteInstanceCommand(instanceKey, instance.QSP.start_slave_until_master_log(),
...
func ChangeMasterCredentials(instanceKey *InstanceKey, creds *modeldomain.ReplicationCredentials) (*Instance, error) {
	instance, err := ReadTopologyInstance(instanceKey)
	if err != nil {
		return instance, log.Errore(err)
	}
...
func ChangeMasterTo(instanceKey *InstanceKey, masterKey *InstanceKey, masterBinlogCoordinates *BinlogCoordinates, skipUnresolve bool, gtidHint modeldomain.OperationGTIDHint) (*Instance, error) {
	instance, err := ReadTopologyInstance(instanceKey)
	if err != nil {
		return instance, log.Errore(err)
	}
...
func ResetReplication(instanceKey *InstanceKey) (*Instance, error) {
	...
	err = ExecuteInstanceCommand(instanceKey, instance.QSP.change_master_to_master_host())
	if err != nil {
		return instance, log.Errore(err)
	}
	err = ExecuteInstanceCommand(instanceKey, instance.QSP.reset_slave_50603_all())
...
func skipQueryClassic(instance *Instance) error {
	err := ExecuteInstanceCommand(&instance.Key, instance.QSP.set_sql_slave_skip_counter())
	return err
}
...
func MasterPosWait(instanceKey *InstanceKey, binlogCoordinates *BinlogCoordinates) (*Instance, error) {
	...
	err = ExecuteInstanceCommand(instanceKey, instance.QSP.select_master_pos_wait(), binlogCoordinates.LogFile, binlogCoordinates.LogPos)
	if err != nil {
		return instance, log.Errore(err)
	}
...
func ReadReplicationCredentials(instanceKey *InstanceKey) (creds *modeldomain.ReplicationCredentials, err error) {
...
func ShowMasterStatus(instance *Instance, instanceKey *InstanceKey) (masterStatusFound bool, executedGtidSet string, err error) {
	...
	err = db.ReadDynamicRows(instance.QSP.show_master_status(), func(m modeldomain.DynamicRow) error {
