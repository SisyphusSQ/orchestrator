...
func updateClusterAliasesUsingReplace() error {
	return metadata.UpdateClusterAliasesUsingReplace(context.Background(), instmodel.DowntimeLostInRecoveryMessage)
}
...
func updateClusterAliasesUsingInsert() error {
	return metadata.UpdateClusterAliasesUsingInsert(context.Background(), instmodel.DowntimeLostInRecoveryMessage)
}
...
func UpdateClusterAliases() error {
	...
	if err := metadata.ExecuteWrite(context.Background(), writeFunc); err != nil {
		return err
	}
	writeFunc = func() error {
...
func ForgetLongUnseenClusterAliases() error {
	...
	instaudit.AuditOperation("forget-clustr-aliases", nil, fmt.Sprintf("Forgotten aliases: %d", rows))
	return err
}
...
func ReplaceAliasClusterName(oldClusterName string, newClusterName string) (err error) {
	...
	}
	{
		...
		if ferr := metadata.ExecuteWrite(context.Background(), writeFunc); ferr != nil {
			err = ferr
		}
	}
	return err
...
// of the alias.
func ReadUnambiguousSuggestedClusterAliases() (result map[string]instmodel.InstanceKey, err error) {
	result = map[string]instmodel.InstanceKey{}

	rows, err := metadata.ReadUnambiguousSuggestedClusterAliases(context.Background())
	for _, row := range rows {
		result[row.SuggestedAlias] = instmodel.InstanceKey{Hostname: row.Hostname, Port: row.Port}
	}
	return result, err
}
