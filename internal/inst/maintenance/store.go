...
func BeginBoundedMaintenance(instanceKey *instmodel.InstanceKey, owner string, reason string, durationSeconds uint, explicitlyBounded bool) (int64, error) {
	...
	if !inserted {
		...
		instaudit.AuditOperation("begin-maintenance", instanceKey, fmt.Sprintf("maintenanceToken: %d, owner: %s, reason: %s", maintenanceToken, owner, reason))
	}
...
func BeginMaintenance(instanceKey *instmodel.InstanceKey, owner string, reason string) (int64, error) {
...
func EndMaintenanceByInstanceKey(instanceKey *instmodel.InstanceKey) (wasMaintenance bool, err error) {
	...
	if affected > 0 {
		// success
		wasMaintenance = true
		instaudit.AuditOperation("end-maintenance", instanceKey, "")
	}
	return wasMaintenance, err
}

// InMaintenance checks whether a given instance is under maintenacne
func InMaintenance(instanceKey *instmodel.InstanceKey) (inMaintenance bool, err error) {
...
func ReadMaintenanceInstanceKey(maintenanceToken int64) (*instmodel.InstanceKey, error) {
	var res *instmodel.InstanceKey
	rows, err := metadata.ReadMaintenanceInstanceKey(context.Background(), maintenanceToken)
	if err == nil && len(rows) > 0 {
		instanceKey, merr := instresolve.NewInstanceKey(rows[0].Hostname, rows[0].Port)
		if merr != nil {
...
func EndMaintenance(maintenanceToken int64) (wasMaintenance bool, err error) {
	...
	if affected > 0 {
		...
		instaudit.AuditOperation("end-maintenance", instanceKey, fmt.Sprintf("maintenanceToken: %d", maintenanceToken))
	}
	return wasMaintenance, err
}
...
func ExpireMaintenance() error {
	{
		...
		if rowsAffected > 0 {
			instaudit.AuditOperation("expire-maintenance", nil, fmt.Sprintf("Purged historical entries: %d", rowsAffected))
		}
	}
	{
		...
		if rowsAffected > 0 {
			instaudit.AuditOperation("expire-maintenance", nil, fmt.Sprintf("Expired bounded: %d", rowsAffected))
		}
	}
	{
		...
		if rowsAffected > 0 {
			instaudit.AuditOperation("expire-maintenance", nil, fmt.Sprintf("Expired dead: %d", rowsAffected))
		}
	}
