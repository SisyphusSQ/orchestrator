...
func GetReplicationAnalysis(clusterName string, hints *dto.ReplicationAnalysisHints) ([]ReplicationAnalysis, error) {
	...
	for _, row := range rows {
		...
		a.AnalyzedInstanceKey = instmodel.InstanceKey{Hostname: row.Hostname, Port: row.Port}
		a.AnalyzedInstanceMasterKey = instmodel.InstanceKey{Hostname: row.MasterHost, Port: row.MasterPort}
		...
		a.AnalyzedInstanceBinlogCoordinates = instmodel.BinlogCoordinates{
			LogFile: row.BinaryLogFile,
			LogPos:  row.BinaryLogPos,
			Type:    instmodel.BinaryLog,
		}
		...
		a.Replicas = *instmodel.NewInstanceKeyMap()
		_ = instresolve.ReadCommaDelimitedList(&a.Replicas, row.SlaveHosts.String)
...
func auditInstanceAnalysisInChangelog(instanceKey *instmodel.InstanceKey, analysisCode AnalysisCode) error {
...
func ReadPeerAnalysisMap() (peerAnalysisMap PeerAnalysisMap, err error) {
	...
	for _, row := range rows {
		instanceKey := instmodel.InstanceKey{Hostname: row.Hostname, Port: row.Port}
