...
func init() {
	go initializeBinlogStorePostConfiguration()
}

func initializeBinlogStorePostConfiguration() {
...
func getInstanceBinlogEntryKey(instance *instmodel.Instance, entry string) string {
...
func getLastPseudoGTIDEntryInBinlog(pseudoGTIDRegexp *regexp.Regexp, instanceKey *instmodel.InstanceKey, binlog string, binlogType instmodel.BinlogType, minCoordinates *instmodel.BinlogCoordinates, maxCoordinates *instmodel.BinlogCoordinates) (*instmodel.BinlogCoordinates, string, error) {
	if binlog == "" {
	...
	binlogCoordinates := instmodel.BinlogCoordinates{LogFile: binlog, LogPos: 0, Type: binlogType}
	...
	for moreRowsExpected {
		...
		err = db.ReadDynamicRows(query, func(m modeldomain.DynamicRow) error {
			...
			if pseudoGTIDMatches(pseudoGTIDRegexp, binlogEntryInfo) {
				if maxCoordinates != nil && maxCoordinates.SmallerThan(&instmodel.BinlogCoordinates{LogFile: binlog, LogPos: m.GetInt64("Pos")}) {
					// past the limitation
					moreRowsExpected = false
					return nil
				}
...
func GetLastPseudoGTIDEntryInInstance(instance *instmodel.Instance, minBinlogCoordinates *instmodel.BinlogCoordinates, maxBinlogCoordinates *instmodel.BinlogCoordinates, exhaustiveSearch bool) (*instmodel.BinlogCoordinates, string, error) {
	...
	for err == nil {
		...
		resultCoordinates, entryInfo, err := getLastPseudoGTIDEntryInBinlog(pseudoGTIDRegexp, &instance.Key, currentBinlog.LogFile, instmodel.BinaryLog, minBinlogCoordinates, maxBinlogCoordinates)
		if err != nil {
			return nil, "", err
		}
...
func GetLastPseudoGTIDEntryInRelayLogs(instance *instmodel.Instance, minBinlogCoordinates *instmodel.BinlogCoordinates, recordedInstanceRelayLogCoordinates instmodel.BinlogCoordinates, exhaustiveSearch bool) (*instmodel.BinlogCoordinates, string, error) {
	...
	for err == nil {
		...
		if resultCoordinates, entryInfo, err := getLastPseudoGTIDEntryInBinlog(pseudoGTIDRegexp, &instance.Key, currentRelayLog.LogFile, instmodel.RelayLog, minBinlogCoordinates, &recordedInstanceRelayLogCoordinates); err != nil {
			return nil, "", err
...
func readBinlogEvent(binlogEvent *BinlogEvent, m modeldomain.DynamicRow) error {
...
func ReadBinlogEventAtRelayLogCoordinates(instanceKey *instmodel.InstanceKey, relaylogCoordinates *instmodel.BinlogCoordinates) (binlogEvent *BinlogEvent, err error) {
	db, err := topology.Open(instanceKey.Hostname, instanceKey.Port)
	if err != nil {
		return nil, err
	}
...
// the last relay log. We must be careful not to scan for Pseudo-GTID entries past the position executed by the SQL thread.
// maxCoordinates == nil means no limit.
func getLastExecutedEntryInRelaylog(instanceKey *instmodel.InstanceKey, binlog string, minCoordinates *instmodel.BinlogCoordinates, maxCoordinates *instmodel.BinlogCoordinates) (binlogEvent *BinlogEvent, err error) {
	if binlog == "" {
	...
	binlogEvent = &BinlogEvent{
		Coordinates: instmodel.BinlogCoordinates{LogFile: binlog, LogPos: 0, Type: instmodel.RelayLog},
	}
...
func GetLastExecutedEntryInRelayLogs(instance *instmodel.Instance, minBinlogCoordinates *instmodel.BinlogCoordinates, recordedInstanceRelayLogCoordinates instmodel.BinlogCoordinates) (binlogEvent *BinlogEvent, err error) {
	// Look for last GTID in relay logs:
	// Since MySQL does not provide with a SHOW RELAY LOGS command, we heuristically start from current
	// relay log (indicated by Relay_log_file) and walk backwards.
...
func searchEventInRelaylog(instanceKey *instmodel.InstanceKey, binlog string, searchEvent *BinlogEvent, minCoordinates *instmodel.BinlogCoordinates) (binlogCoordinates, nextCoordinates *instmodel.BinlogCoordinates, found bool, err error) {
	binlogCoordinates = &instmodel.BinlogCoordinates{LogFile: binlog, LogPos: 0, Type: instmodel.RelayLog}
	nextCoordinates = &instmodel.BinlogCoordinates{LogFile: binlog, LogPos: 0, Type: instmodel.RelayLog}
	if binlog == "" {
...
func SearchEventInRelayLogs(searchEvent *BinlogEvent, instance *instmodel.Instance, minBinlogCoordinates *instmodel.BinlogCoordinates, recordedInstanceRelayLogCoordinates instmodel.BinlogCoordinates) (binlogCoordinates, nextCoordinates *instmodel.BinlogCoordinates, found bool, err error) {
	// Since MySQL does not provide with a SHOW RELAY LOGS command, we heuristically start from current
	// relay log (indicated by Relay_log_file) and walk backwards.
...
func SearchEntryInBinlog(pseudoGTIDRegexp *regexp.Regexp, instanceKey *instmodel.InstanceKey, binlog string, entryText string, monotonicPseudoGTIDEntries bool, minBinlogCoordinates *instmodel.BinlogCoordinates) (instmodel.BinlogCoordinates, bool, error) {
	binlogCoordinates := instmodel.BinlogCoordinates{LogFile: binlog, LogPos: 0, Type: instmodel.BinaryLog}
	if binlog == "" {
...
func SearchEntryInInstanceBinlogs(instance *instmodel.Instance, entryText string, monotonicPseudoGTIDEntries bool, minBinlogCoordinates *instmodel.BinlogCoordinates) (*instmodel.BinlogCoordinates, error) {
	...
	if found {
		...
		return coords.(*instmodel.BinlogCoordinates), nil
	}
	...
	for {
		...
		if instance.ReplicaRunning() {
			for {
				...
				instance, err = instdiscovery.ReadTopologyInstance(&instance.Key)
				if err != nil {
					break
				}
...
func readBinlogEventsChunk(instanceKey *instmodel.InstanceKey, startingCoordinates instmodel.BinlogCoordinates) ([]BinlogEvent, error) {
	events := []BinlogEvent{}
	db, err := topology.Open(instanceKey.Hostname, instanceKey.Port)
	if err != nil {
		return events, err
	}
	commandToken := math.TernaryString(startingCoordinates.Type == instmodel.BinaryLog, "binlog", "relaylog")
...
func getNextBinlogEventsChunk(instance *instmodel.Instance, startingCoordinates instmodel.BinlogCoordinates, numEmptyBinlogs int) ([]BinlogEvent, error) {
...
func formatEventCleanly(event BinlogEvent, length *int) string {
	return fmt.Sprintf("%+v %+v; %+v", instmodel.RightPadCoordinates(event.Coordinates, length), event.EventType, strings.Split(strings.TrimSpace(event.Info), "\n")[0])
}
...
func special56To57filterProcessing(instance *instmodel.Instance, other *instmodel.Instance) (bool, bool, error) {
	// be paranoid
	...
	master, _, err := instinventory.ReadInstance(&instance.MasterKey)
	if err != nil {
...
func GetNextBinlogCoordinatesToMatch(
	...
	instanceCoordinates instmodel.BinlogCoordinates,
	recordedInstanceRelayLogCoordinates instmodel.BinlogCoordinates,
	maxBinlogCoordinates *instmodel.BinlogCoordinates,
	other *instmodel.Instance,
	otherCoordinates instmodel.BinlogCoordinates) (*instmodel.BinlogCoordinates, int, error) {
	...
	fetchNextEvents := func(binlogCoordinates instmodel.BinlogCoordinates) ([]BinlogEvent, error) {
	...
	fetchOtherNextEvents := func(binlogCoordinates instmodel.BinlogCoordinates) ([]BinlogEvent, error) {
	...
	var (
		...
		lastConsumedEventCoordinates instmodel.BinlogCoordinates
	)
...
func GetPreviousGTIDs(instanceKey *instmodel.InstanceKey, binlog string) (previousGTIDs *gtid.OracleGtidSet, err error) {
	if binlog == "" {
	...
	err = db.ReadDynamicRows(query, func(m modeldomain.DynamicRow) error {
		...
		if eventType == "Previous_gtids" {
			var e error
			if previousGTIDs, e = gtid.NewOracleGtidSet(m.GetString("Info")); e != nil {
				return e
			}
		}
		return nil
