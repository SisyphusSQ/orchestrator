...
// Package binlog owns binlog events, cursors, and correlation queries.
package binlog
...
type BinlogEvent struct {
	Coordinates  instmodel.BinlogCoordinates
	NextEventPos int64
	EventType    string
	Info         string
}

func (event *BinlogEvent) NextBinlogCoordinates() instmodel.BinlogCoordinates {
	return instmodel.BinlogCoordinates{LogFile: event.Coordinates.LogFile, LogPos: event.NextEventPos, Type: event.Coordinates.Type}
}
...
type BinlogEventCursor struct {
	...
	fetchNextEvents   func(instmodel.BinlogCoordinates) ([]BinlogEvent, error)
	nextCoordinates   instmodel.BinlogCoordinates
}
...
func NewBinlogEventCursor(startCoordinates instmodel.BinlogCoordinates, fetchNextEventsFunc func(instmodel.BinlogCoordinates) ([]BinlogEvent, error)) BinlogEventCursor {
	events, _ := fetchNextEventsFunc(startCoordinates)
	var initialNextCoordinates instmodel.BinlogCoordinates
	if len(events) > 0 {
...
// entries of both.
func (cursor *BinlogEventCursor) getNextCoordinates() (instmodel.BinlogCoordinates, error) {
