...
func ReadAllHostnameUnresolvesRegistrations() (registrations []HostnameRegistration, err error) {
	...
	for _, unresolve := range unresolves {
		registration := NewHostnameRegistration(&instmodel.InstanceKey{Hostname: unresolve.hostname}, unresolve.unresolvedHostname)
...
func WriteHostnameUnresolve(instanceKey *instmodel.InstanceKey, unresolvedHostname string) error {
	writeFunc := func() error {
...
func DeleteHostnameUnresolve(instanceKey *instmodel.InstanceKey) error {
	writeFunc := func() error {
...
// readUnresolvedHostname reverse-reads hostname resolve. It returns a hostname which matches given pattern and resovles to resolvedHostname,
// or, in the event no such hostname is found, the given resolvedHostname, unchanged.
// ReadHostnameIPs reads the last persisted IPv4 and IPv6 addresses for hostname.
