...
// Package candidate owns failover candidate suggestions and expiry.
package candidate
...
type CandidateDatabaseInstance struct {
	Hostname            string
	Port                int
	PromotionRule       instmodel.CandidatePromotionRule
	LastSuggestedString string
	PromotionRuleExpiry string // generated when retrieved from database for consistency reasons
}

func NewCandidateDatabaseInstance(instanceKey *instmodel.InstanceKey, promotionRule instmodel.CandidatePromotionRule) *CandidateDatabaseInstance {
...
func (cdi *CandidateDatabaseInstance) Key() *instmodel.InstanceKey {
	return &instmodel.InstanceKey{Hostname: cdi.Hostname, Port: cdi.Port}
}
