...
func AuditOperation(auditType string, instanceKey *instmodel.InstanceKey, message string) error {
	if instanceKey == nil {
		instanceKey = &instmodel.InstanceKey{}
	}
	clusterName := ""
	if instanceKey.Hostname != "" {
		rows, _ := metadata.ReadInstanceClusterName(context.Background(), instanceKey.Hostname, instanceKey.Port)
		if len(rows) > 0 {
			clusterName = rows[0]
		}
	}
...
func ReadRecentAudit(instanceKey *instmodel.InstanceKey, page int) ([]Audit, error) {
