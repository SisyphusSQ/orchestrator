...
func PopulateGroupReplicationInformation(instance *instmodel.Instance, db *topology.Client) error {
	...
	for _, member := range members {
		...
		groupMemberKey, err := instresolve.NewInstanceKey(member.Host, int(member.Port))
		if err != nil {
		...
		if member.Role == instmodel.GroupReplicationMemberRolePrimary && groupMemberKey != nil {
	...
	if !foundGroupPrimary {
		err = instinventory.ReadReplicationGroupPrimary(instance)
		if err != nil {
