// Package discovery owns topology discovery runtime state and probes.
package discovery
...
type deadInstancesFilter struct {
	deadInstances      map[instmodel.InstanceKey]deadInstance
...
func init() {
	...
	DeadInstancesFilter.deadInstances = make(map[instmodel.InstanceKey]deadInstance)
...
func (f *deadInstancesFilter) RegisterInstance(instanceKey *instmodel.InstanceKey) {
...
func (f *deadInstancesFilter) UnregisterInstance(instanceKey *instmodel.InstanceKey) {
	f.deadInstancesMutex.Lock()
	defer f.deadInstancesMutex.Unlock()
...
func (f *deadInstancesFilter) InstanceRecheckNeeded(instanceKey *instmodel.InstanceKey) (bool, bool) {
