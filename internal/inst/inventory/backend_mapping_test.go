...
func TestReadInstanceRowMapsTypedBackendDTO(t *testing.T) {
	row := modeldomain.BackendInstanceRecord{
		Hostname:                      "db.example",
		Port:                          3307,
		Version:                       "8.0.42",
		...
		PromotionRule:                 string(instmodel.PreferPromoteRule),
		...
		ReplicationGroupMemberState:   instmodel.GroupReplicationMemberStateOnline,
		ReplicationGroupMemberRole:    instmodel.GroupReplicationMemberRoleSecondary,
	...
	if sql.NullInt64(instance.ReplicationLagSeconds) != row.SlaveLagSeconds || sql.NullInt64(instance.SecondsSinceLastSeen) != row.SecondsSinceLastSeen {
...
func TestReadInstanceRowPreservesLegacyNullFreshnessSemantics(t *testing.T) {
	instance := readInstanceRow(modeldomain.BackendInstanceRecord{})
	if !instance.IsUpToDate || !instance.IsRecentlyChecked {
