...
func seedMetadataRow(t *testing.T, database *sql.DB, table string, number int, explicitID bool) {
	t.Helper()
	columns, err := metadataColumns(context.Background(), database, table)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, column := range columns {
		...
		names = append(names, quoteMetadataIdentifier(column.name))
		marks = append(marks, "?")
	...
	if _, err := database.Exec("INSERT INTO "+quoteMetadataIdentifier(table)+" ("+strings.Join(names, ",")+") VALUES ("+strings.Join(marks, ",")+")", values...); err != nil {
		t.Fatalf("seed %s: %v", table, err)
	}
}

func snapshotRowMaps(t *testing.T, database *sql.DB, table string) []map[string]CellData {
	t.Helper()
	data, err := ScanTableContext(context.Background(), database, table)
	if err != nil {
		t.Fatal(err)
	}
	result := []map[string]CellData{}
	for _, row := range data.Data {
		record := map[string]CellData{}
	...
	for _, source := range []string{"canonical-v1", "legacy-v1"} {
		t.Run(source, func(t *testing.T) {
			...
			for table, want := range before {
				...
				var maximum int64
				if err := database.QueryRow("SELECT MAX(id) FROM " + quoteMetadataIdentifier(table)).Scan(&maximum); err != nil {
					t.Fatal(err)
				}
...
func TestMetadataIDMigrationRejectsUnexpectedShapeBeforeWrites(t *testing.T) {
	...
	columns, err := metadataColumns(ctx, database, "audit")
	if err != nil {
		t.Fatal(err)
	}
	for _, column := range columns {
...
func TestSnapshotLocalIDsCannotOverwriteOtherBusinessRows(t *testing.T) {
	...
	data.Data[0] = append(data.Data[0], CellData{String: "1", Valid: true})
...
func TestSnapshotRejectsMissingOrAmbiguousIdentity(t *testing.T) {
	for _, test := range []struct {
		...
		row := make(RowData, len(test.columns))
		if _, err := snapshotIdentityColumns(test.table, NamedResultData{Columns: test.columns, Data: ResultData{row}}, false); err == nil {
