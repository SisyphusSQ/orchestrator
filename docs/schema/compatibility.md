...
| SQLite | 由同一权威 DDL 转换 | Go 测试执行 50 表初始化、重复执行和布局识别 | SQLite 不保存 MySQL 表/字段注释和字符集属性 |
...
- `mysql.sql` 只用于空库；存量结构统一使用显式迁移入口，TiDB/OceanBase 还必须走对应平台的变更与回读流程。

## 自增主键 v2

MySQL 目标为每表 `id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY`；SQLite 转换为 `INTEGER PRIMARY KEY`，省略 id 时由数据库分配值。SQLite 的 ROWID 分配不承诺删除后的编号永不复用；不能把新增代理 id 用作跨节点业务身份。

旧版自动编号改名保留数值；原业务主键以同序唯一索引保留。MySQL 存量使用 CHANGE COLUMN 或 ADD COLUMN / DROP PRIMARY KEY / ADD PRIMARY KEY / ADD UNIQUE INDEX；SQLite 使用逐表事务复制。迁移结构保留旧库的业务列类型、默认值和原索引名称，不声称存量库物理结构与空库的注释/排序规则完全相同。

外部测试各需要一份独立、预先为空的数据库，测试会写入结构和 fixture 并保留供回读：
