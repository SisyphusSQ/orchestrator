...
HTTP Access | Must only access the leader (can be enforced by proxy or `orch`) | May access any healthy node (can be enforced by proxy). For read consistency always best to speak to leader only (can be enforced by proxy or `orch`)
Command line | HTTP/API access (e.g. `curl`, `jq`) or `orch` Go client which wraps common HTTP /API calls with familiar command line interface | HTTP/API, and/or `orch` Go client.
Install | `orchestrator` service on service nodes only. `orch` Go client anywhere (requires access to HTTP/API). | `orchestrator` service on service nodes. `orch` Go client anywhere (requires access to HTTP/API).
...
No proxy | Use `orch` with all `orchestrator` backends. `orch` will direct traffic to master. | Use `orch` with all `orchestrator` backends. `orch` will direct traffic to master.
