...
func TestHTTPRaftLifecycle(t *testing.T) {
	...
	for i := range 3 {
		...
		config := map[string]any{"BackendDB": "sqlite", "SQLite3DataFile": dbfile, "ListenAddress": strings.TrimPrefix(endpoint, "http://"), "HTTPAdvertise": endpoint, "HostnameResolveMethod": "none", "RaftEnabled": true, "RaftNodeID": fmt.Sprintf("e2e-%d", i), "RaftBind": address, "RaftAdvertise": address, "RaftDataDir": home, "Debug": false, "EnableSyslog": false, "AuditToSyslog": false, "InstancePollSeconds": 60}
		...
		nodes = append(nodes, node{endpoint, address, cmd, db})
	}
	...
	t.Log("follower routing, replicated SQL readback, multiple endpoints, leader replacement and no-quorum rejection passed")
}
